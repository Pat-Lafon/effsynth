...
let sort_layout = fun s ->
          ...
          | _ -> failwith "sort_layout in z3_encode.ml is not exhaustice"
...
let mkDefaultContext () =
  let cfg = [("model", "true"); ("proof", "true"); ("timeout", "600000")(* ;("smt.macro-finder","true") *)] in
...
let mkSet (name,sorts) =
  let _nargs = Vector.length sorts in
  ...
  let pred =
    ...
    let _sortStr = fun _ -> (List.fold_left (fun acc s ->  acc^","^s) "(" sortStrs)^")" in
    let _errMsg = (fun _ -> "Type Mismatch. Set: "^name^".\n") in
...
let mkStrucRel (name,sorts) =
  ...
  let [@warning "-8"] Set {ty;pred} = mkSet (name,sorts) [@warning "+8"] in
  let rel = fun ast ->
...
let mkMStrucRel (name,sorts) =
  ...
  let _domainTys = List.rev (List.tl (List.rev sorts)) in
...
let mkEmptySet sorts =
  let [@warning "-8"] (Set {ty;pred} as set) = mkSet (genSetName (),sorts) [@warning "+8"] in
...
let mkAddition = function
        ...
        | (s1,Null) -> s1
        | (Set {ty=sorts1;pred=pred1}, Set {ty=sorts2;pred=pred2}) ->
          let [@warning "-8"] Set {ty;pred} as s  = mkSet (genSetName (), sorts1) [@warning "+8"] in
...
let mkCrossPrd = function
  ...
  | (Set {ty=sorts1;pred=pred1}, Set {ty=sorts2;pred=pred2}) ->
      ...
      let [@warning "-8"] Set {ty;pred} as s = mkSet (genSetName (), sorts) [@warning "+8"] in
      let _ = assertSetProp (ty, fun bvAsts ->
...
let mkQCrossPrd = fun (sets) ->
  ...
  let [@warning "-8"]  (dsorts,lens,rsorts) = h (Vector.unzip(List.map (fun (Set {ty;_}) -> (Vector.sub (ty,0), Vector.dropPrefix (ty,1))) sets)) [@warning "+8"] in
  ...
  let _ = assertSetProp (ty, fun bvAsts ->
      ...
      let [@warning "-8"]  fnapps = Vector.map2 (sets, bvss,
                                fun (Set {pred;_},bvs) -> pred bvs) [@warning "+8"] in
...
let mkQSingletonSet sorts =
  ...
  let [@warning "-8"] Set {ty;pred} as set  = mkSet (genSetName (),qSorts) [@warning "+8"] in
...
let mkQStrucRelApp (SR {ty;rel}) =
    Set {ty=ty; pred=
                                                 fun bvs ->
                                                   ...
                                                   let [@warning "-8"] Set {pred;_} = rel bv0 [@warning "+8"]
                                                   in
                                                   pred bvs'
                                       }
...
let mkConstEqAssertion (AST (x1,s1) as ast1, AST (x2,s2)) =
  (assert (typeCheckAst (ast1,s2)); mk_eq x1 x2)

let mkConstGtAssertion (AST (x1,s1) as ast1, AST (x2,s2)) =
  (assert(typeCheckAst (ast1,s2)); mk_gt x1 x2)
