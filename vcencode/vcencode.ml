...
module Message = struct
    ...
    let show (s:string) = if !debug then (Printf.originalPrint "%s" ("\n"^(String.make !depth ' ')^s))
...
[@@@ warning "-8"]
let discharge (VC.T (tydbinds, anteP, conseqP)) typenames qualifiers =
  ...
  let () = Message.show ("GAMMA in VCE:") in
  let () = VC.print_for_tybinds tydbinds in

  let _constMap = ConstMap.empty in
  let _relMap = RelMap.empty in
  ...
  let getSRCodomainSort relMap rid  : (sort) =
    ...
    let encodeTyD (tyMap, constMap, relMap) tyD  =
      try
        let () = Message.show ("encodeTyD "^TyD.toString tyD) in
        (tyMap, constMap, relMap, (TyMap.find tyMap tyD))
      with
      | TyMap.TyDNotFound _ ->
        let () = Message.show ("@@@@@"^(TyD.toString tyD)) in
      ...
      let encodeConst (tyMap, constMap, relMap) (v,tyd)  =
        ...
        let () = Message.show ("encodeConst "^vstr^ "\n ") in
  ...
  (*        let (tyMap, constMap, relMap, const) = encodeConst (tyMap, constMap, relMap) ((Var.fromString "var_i8"), (Ty_ref (Ty_list Ty_int))) in *)
          ...
          * heap -> ref list int -> int *)
          ...
          let (tyMap, constMap, relMap, sort_heap) = encodeTyD (tyMap, constMap, relMap) (Ty_heap) in
          let () = Message.show ("Sort for Heap >>>> "^sortToString sort_heap) in
          (*TODO :: These Manual Mappings must be automated*)
      ...
      let processTyDBind (tyMap, constMap, relMap) (v,tyd) =
          ...
          let () = Message.show ("processTyDBind VCE "^(Var.toString v)) in
          ( match tyd with
        (*
         ...
         *)
          ...
          |_ ->
              let () = Message.show ("processTyDBind consEncoding Case ") in

              let (tyMap, constMap, relMap, _) = encodeConst (tyMap, constMap, relMap) (v,tyd) in
            let () = Message.show ("processTyDBind consEncoding done ") in
    ...
    let processTypenames (tyMap, constMap, relMap) tyd =
        let () = Message.show ("processTypenames VCE "^(tyd)) in
    ...
    let processQualifiers (tyMap, constMap, relMap) qual =
        let () = Message.show ("processQualifiers VCE "^(SpecLang.RelSpec.Qualifier.toString qual)) in
    ...
    let _processPrimEq (tyMap, constMap, relMap) (primR, def) =
         let () = Message.show ("processPrimEq PrimEq  ") in
    ...
    let rec encodeBasePred (tyMap, constMap, relMap) (bp)  =
      ...
      let () = Message.show ("******encodeBasePred *****") in

        let
          open BP in
          let encodeBaseExpr bexp =
            let () = Message.show "$$$$$$$$$$$$$$$$$-encdode BaseExp " in

            match bexp with
             (Int i) ->
              let () = Message.show "$$$$$$$$$$$$$$$$$-encdode BaseExp Int " in
            ...
            | Bool true ->
              let () = Message.show "$$$$$$$$$$$$$$$$$-encdode BaseExp BoolT " in

                const_true
            | Bool false ->
              let () = Message.show "$$$$$$$$$$$$$$$$$-encdode BaseExp BoolF" in

              const_false
            | Var v ->
              let () = Message.show ("$$$$$$$$$$$$$$$$$-encdode BaseExp Var %%%%%%%%%%%%%%%%%%%%%%"^(Ident.name v)) in
              getConstForVar constMap v
            | Str s ->
              let () = Message.show ("$$$$$$$$$$$$$$$$$-encdode BaseExp Str "^(s)) in
        ...
        in
          ...
          match bp with
           Eq (e1,e2) ->
            let () = Message.show ("$$$$$$$$$$$$$$$$$ BasePredicate Eq  "^(BP.toString bp)) in
              mkConstEqAssertion (encodeBaseExpr e1, encodeBaseExpr e2)
           | Iff (bp1,bp2) ->
              let () = Message.show  ("$$$$$$$$$$$$$$$$$ BasePredicate Iff  "^ (BP.toString bp)) in
     ...
     let rec encodeRelExpr (tyMap, constMap, relMap) (e) =
      ...
      let () = Message.show ("******encodeRelExpr *****") in
      let () = Message.show ("RelExp  "^ (RelLang.exprToString e)) in

        let open RelLang in
        let encodeRelElem (tyMap, constMap, relMap) = fun x ->
          let () = Message.show ("******encodeRelElem  *****") in
        ...
        in
          let () = Message.show ("******encodeRelExpr here *****") in

         match e with
            V elm ->
                 let () = Message.show ("******encodeRelExpr T *****") in
           ...
           |T els ->
            ...
            | ADD (e1, e2) ->
             ...
             | R (RInst {rel=rid;_},v) ->
              ...
              let ()  = Message.show ("Type "^str_ty) in
              let _const = getConstForVar constMap v in
              let out = mkStrucRelApp (
                (getStrucRelForRelId relMap rid), (getConstForVar constMap v)) in
              out
            | MultiR (RInst {rel=rid;_}, args) ->

                 let ()  = Message.show ("*******encodeRelExpr MultiR******** ") in
                let srforR = getStrucRelForRelId relMap rid in
              ...
              let ()  = Message.show ("arguments "^(List.fold_left (fun acca (arg_exp)-> (acca^", "^Expr.to_string arg_exp)) "Arg Exps " arg_exp_list )) in
              let ()  = Message.show ("arguments Sorts "^(List.fold_left(fun accs s -> (accs^", "^Z3.Sort.to_string s) ) "Arg Sorts " arg_sort_list)) in
               (*We need to create a set by application*)
               let ufApp = mkMStrucRelApp  (srforR, cosnt_for_arg_list) in
                let ()  = Message.show ("Uninterpreted Function App Encoding Done ") in
                ufApp
      ...
      let rec encodeNumericExpr (tyMap, constMap, relMap) (e) =
        let () = Message.show ("******encodeNumericExpr *****") in
        let open RelLang in
        let encodeNumericElem (tyMap, constMap, relMap) = fun x ->
          match x with
            | (Int i) ->
                    let () = Message.show "Encoding Numeric Elem \n " in
                    let res = mk_Numeric_constant i in
                    let () = Message.show "Encoding Numeric Elem Success \n " in
                      res
            ...
            | Var v ->
                        let () = Message.show "Encoding Numeric Elem Var Case \n " in
                        let () = Message.show (Ident.name v) in
        ...
        |  V elm -> encodeNumericElem (tyMap, constMap, relMap) elm
          |T els ->
                ...
                let () = Message.show ("******encodeNumericExpr T *****") in
                (
                match Vector.length els with
            ...
            | R (RInst {rel=rid;_},v) ->
              let () = Message.show ("******encodeNumericExpr R *****") in
              ...
              let ()  = Message.show ("Type "^str_ty) in
              let () = Message.show ("argument Name"^(Ident.name v)) in

              let argument = getConstForVar constMap v in
              let _astLayout = ast_layout argument in
              let (arg_exp, arg_sort) =ast_expr_sort_pair argument in


              let reln_required_arg_sort = sortToZ3Sort arg_sort in
              let ()  = Message.show ("argument Sort "^(Z3.Sort.to_string reln_required_arg_sort)) in


              let relate = mk_Integer_func_decl (RelId.toString rid, [reln_required_arg_sort], (mk_int_sort())) in
              let ()  = Message.show ("relation "^(FuncDecl.to_string relate)) in

              let ()  = Message.show ("arguments "^(Expr.to_string arg_exp)) in
              let ()  = Message.show ("argument Sort "^(Z3.Sort.to_string reln_required_arg_sort)) in

               let relApp = mk_Integer_rel_app  (relate, [arg_exp]) in
                let ()  = Message.show ("relation App Done ") in
                relApp
             (*Define a multi argument Uninterpreted function *)
           | MultiR (RInst {rel=rid;_}, args) ->
                let ()  = Message.show ("*******encodeNumericExpr MultiR******** "^(RelId.toString rid)) in
                let srforR = getStrucRelForRelId relMap rid in
                let MSR{mty;mrel} = srforR in
               let str_ty  = (List.fold_left (fun acc s -> acc^(sortToString s)^" :-> " ) "{" mty )^" }" in
              ...
              let ()  = Message.show ("uf "^(FuncDecl.to_string uf)) in
              let ()  = Message.show ("arguments "^(List.fold_left (fun acca (arg_exp)-> (acca^", "^Expr.to_string arg_exp)) "Arg Exps " arg_exp_list )) in
              let ()  = Message.show ("arguments Sorts "^(List.fold_left(fun accs s -> (accs^", "^Z3.Sort.to_string s) ) "Arg Sorts " arg_sort_list)) in
      ...
      let encodeNumericGrtAssertion (tyMap, constMap, relMap) (e1, e2) =
        let () = Message.show ("******encodeNumericGrtAssertion *****") in
        let encoding_lhs =  encodeNumericExpr (tyMap, constMap, relMap) e1 in
        let () = Message.show ("******encodeNumericGrtAssertion LHS done *****") in
        let () = Message.show ("******encodeNumericEqAssertion RHS  *****") in
        let encoding_rhs = encodeNumericExpr (tyMap, constMap, relMap) e2 in
        let () = Message.show ("******encodeNumericEqAssertion RHS done *****") in
        let () = Message.show ("******encodeNumericEqAssertion creating EQ  *****") in
        let gt_assertion = mk_Integer_gt (encoding_lhs,  encoding_rhs) in
        let () = Message.show ("******encodeNumericEqAssertion creating EQ  DONE*****") in
      ...
      let encodeNumericEqAssertion (tyMap, constMap, relMap) (e1, e2) =
            let () = Message.show ("******encodeNumericEqAssertion *****") in

          let encoding_lhs =  encodeNumericExpr (tyMap, constMap, relMap) e1 in
            let () = Message.show ("******encodeNumericEqAssertion LHS done *****") in
      ...
      in
       let encodeRelPred (tyMap, constMap, relMap) (rp:RP.t)  =
          let () = Message.show ("******encodeRelPred  *****"^(RP.toString rp)) in
          let  open RelLang in
        ...
        match  rp with
         | NEq (e1, e2) ->
              (match (e1, e2) with
                ...
                | (_, SUBS(_,_))
                | (SUBS(_,_), _)->
                         let () = Message.show ("NUMERICAL EXPRESSION SUBS "^(RelLang.exprToString (e1) )^" = "^(RelLang.exprToString (e2) ) ) in
                      encodeNumericEqAssertion (tyMap, constMap, relMap) (e1, e2)


                (*for R and MultiR*)
                |(R (rie1, id1), R (rie2, id2)) ->

                     let () = Message.show ("Case :: Both Rel Exp ") in
                ...
                |(R (rie, id), _) ->
                 ...
                 | (_, R (rie, id))  ->
                      let RInst {rel;_} = rie in
                      let relId = RelId.toString rel in
                      let () = Message.show ("NUMERICAL EXPRESSION CASE 2"^relId) in
                       let codomainsort_r = getSRCodomainSort relMap relId in
                ...
                | (MultiR (rie1, idl1), MultiR (rie2, idl2)) ->
                ...
                |(MultiR (rie, id), _) ->
                    let () = Message.show ("Case NEq (MR , Other) ") in
                    let RInst {rel;_} = rie in
                    let relId = RelId.toString rel in
                    let codomainsort_r = getSRCodomainSort relMap relId in
        ...
        |Eq (e1,e2) -> (*if either of e1 or e2 is an arithmatic relation like rlen, then make arithmeticEqAssertion*)
              (match (e1, e2) with
                ...
                | (_,_) ->
                   let () = Message.show ("RELATIONAL EXPRESSION CASE "^(RelLang.exprToString ( e1) )^" = "^(RelLang.exprToString (e2) )) in
                      mkSetEqAssertion (f e1, f e2)
            )
        |Grt ( e1, e2) ->
              (match (e1, e2) with
                | (* simplest case*) (T el1, T el2)  ->
                  let () = Message.show ("singleton Set Eq CASE "^(RelLang.exprToString ( e1) )^" = "^(RelLang.exprToString (e2) )) in
                ...
                |(MultiR (rie, id), _) ->
                    let () = Message.show ("encode RelPredicate Grt (MR , Other)") in
                    let RInst {rel;_} = rie in
                    let relId = RelId.toString rel in
                    let codomainsort_r = getSRCodomainSort relMap relId in
                    let () = Message.show (">>>>") in
       ...
       let encodeSimplePred (tyMap, constMap, relMap) (sp ) =
         ...
         let () = Message.show ("******encodeSimplePred *****") in
      ...
      let rec encodeVCPred (tyMap, constMap, relMap) vcp =
              ...
              let () = Message.show ("******encodeVCPred *****") in
      ...
      let rec assertVCPred  (tyMap, constMap, relMap) vcp =
       let () = Message.show ("******assertVCPred *****") in
      ...
      let () = Message.show ("# of Z3 expressions "^(string_of_int (List.length expressions_list))) in
      let () = Message.show_with_newlines ("\nsolver: \n"^(Solver.to_string solverDischarged)) in
