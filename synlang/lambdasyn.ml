...
let satHypothesis (t : path) (hypothesis : path) : bool = true
...
and monExp_toOcaml (m:monExp) : string =
    match m with
    | Evar v -> v
    | Ematch ({expMon=matchingExp; ofType}, caselist) ->
        let caselist_toOcaml =
            List.fold_left (fun accstr casei -> (accstr^" \n | "^(caseExp_toOcaml casei))) " " caselist
        in
        "match (" ^ monExp_toOcaml matchingExp ^ ") with " ^ caselist_toOcaml
    | Eapp (fun1, arglist) -> (monExp_toOcaml fun1) ^ List.fold_left (fun acc a -> acc ^ " (" ^ (monExp_toOcaml a) ^ ")") " " arglist
    | Eite (bi, ttr, tfl) ->
            ("if ("^(monExp_toOcaml bi)^" ) then ("^(monExp_toOcaml ttr)^")  else ("^(monExp_toOcaml tfl))^")"
    | _ -> failwith "monExp_toOcaml: unsupported"
...
and typedMonExp_toOcaml (t:typedMonExp) : string =
    let {expMon; ofType} = t in
    "let rec goal "^(RefTy.toOcaml ofType) ^" = \n " ^ monExp_toOcaml expMon
...
and caseExp_toOcaml (t : caseExp) : string =
    let args = (match t.args with
        | h::t -> let argsToString = List.fold_left (fun accstr argi -> (accstr^" ,"^(Var.toString argi))) (Var.toString h) t in " ( "^(argsToString)^" )"
        | [] -> "") in
    ((Var.toOcaml t.constuctor)^args^" -> "^(monExp_toOcaml t.exp.expMon))
