...
let generateNilConstraints (macthedArg) =
      ...
      in
      let () = Message.show ("Nil Length "^(Predicate.toString nil_length)) in
      nil_length
...
let generateConsConstraints (macthedArg : Var.t) (arg_x: Var.t) (arg_xs : Var.t) =
  ...
  let () = Message.show ("Cons Length "^(Predicate.toString cons_length)) in
...
let typecheck (gamma : Gamma.t) (sigma:Sigma.t) (delta : Predicate.t)
    ...
    let () = Message.show ("Typechecking "^(Syn.monExp_toString t)) in
    let () = Message.show ("Against "^(RefTy.toString spec)) in
    match t with
              ...
              | RefTy.Arrow ((_,_), _) ->
                ...
                *)
                ...
                let () = List.iter(fun (vi, vi_type, phi_i) ->
                          Message.show ("Argument "^(vi)^" :: "^(RefTy.toString vi_type)^" | "^(Predicate.toString phi_i) )) actualsTypes in
                ...
                let appType = RefTy.applySubsts subs retTy in
                 ...
                 let () = Message.show ("AppType "^(RefTy.toString appType)) in

                (*the subtyping check*)

                let vc = VC.fromTypeCheck gamma (delta::delta_extended) (appType, spec) in
                let () = VC.print_for_vc_t vc in

                (*make a direct call to the SMT solver*)
                let vcStandard = VC.standardize vc in
                let () = Message.show ("Standardized VC:") in
                let () = VC.print_for_vc_stt vcStandard in
                let () = VCE.Message.depth := !Message.depth +1 in
...
(*                 let () =
                Message.show ("POST NON APPLIED "^(P.toString phi2')) in
 *)
...
let rec accumulatePathType remaining_path acc_gamma acc_delta acc_type =
  match remaining_path with
                   ...
                   (match funType with
                     ...
                     | RefTy.Arrow ((_,_), _) ->
                          ...
                          *)
                          let subs = List.combine actuals formals in
                          (*create the type [actuals/foramls] retTy*)
                         let () = Message.show "Actuals" in
                         let () = List.iter (fun vi ->
                                Message.show (Var.toString vi)) actuals in
                         let () = Message.show "Formals" in
                         let () = List.iter (fun vi ->
                                Message.show (Var.toString vi)) formals in
                          let appType = RefTy.applySubsts subs retTy in
          ...
          *)
          match retVarMonExp with
            | Syn.Evar (v_ret) ->
              ...
              let () = Message.show ("Lifted Type"^ (RefTy.toString liftedType)) in
    ...
    try
      ...
      let () = Message.show ("Found a type for the path in the PMap") in
      (* let () = Message.show (RefTy.toString foundpType) in *)
    ...
    with
      Knowledge.NoMappingForVar e ->
          ...
          *)
          ...
          let _bv_x = Var.get_fresh_var "x" in
          ...
          let _pre_h_v_h' = VC.apply pre_h' [(bv_h', Ty_heap)] in
...
let typeCheckPath ptypeMap gammaMap sigmaMap deltaPred
    ...
    let (gammaMap, deltaPred, ptypeMap, path_type) =
	 ...
	 (* let () = Message.show ("Show Type for path "^(Syn.pathToString path))  in
   let () = Message.show ((RefTy.toString path_type)) in  *)
  ...
  let baseSpec = RefTy.toTyD tspec in
    ...
    if ((not (Effect.isSubEffect effi eff))
      || (not (TyD.sametype  basePath baseSpec) && not (TyD.Ty_unknown == baseSpec)))
          ...
          let () = Message.show ("Found Path Type "^(RefTy.toString path_type)) in
          let () = Message.show ("Compared Against Spec "^(RefTy.toString spec)) in
	        ...
	        (*make a direct call to the SMT solver*)
	        let vcStandard = VC.standardize vc in
	        let () = Message.show (VC.string_for_vc_stt vcStandard) in
	        let result = VCE.discharge vcStandard typenames qualifiers  in
	        match result with
	        | VCE.Success ->
