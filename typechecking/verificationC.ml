...
module Message = struct
    ...
    let show (s:string) = if !debug then Printf.printf "%s" ("\n"^ (String.make !depth ' ') ^s)
...
let remove_from_gamma (vi) (g: vctybinds) : vctybinds = List.remove_assoc vi g
...
let print_for_tybinds (gs:vtydbind list) = Message.show_with_newlines (string_tybinds gs)
...
let print_for_vc_t (vc:t) = Message.show_with_newlines (string_for_vc_t vc)
...
let print_for_vc_stt (vc : standardt) = Message.show_with_newlines (string_for_vc_stt vc)
...
(*    | _ -> ([], coercePTtoT pred) (* May need havoc here.*) *)
...
let prepend_preds (preds : predicates) (p : P.t) = List.concat [preds;[p]]
...
(*                 let () = Printf.printf "%s" ("\n Original Annotatetd pre "^P.toString p2) in  *)
               ...
               (*  let () = Printf.printf "%s" ("\n Applied Annotated pre "^P.toString p2_temp) in *)
                ...
                (*Post1 => pre2*)
                let _phi_annot_imp_pre = P.If (p2_temp, p1_temp) in
